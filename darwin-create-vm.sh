...
# Copies install scripts to VM
scp -i ./id_ed25519 -o StrictHostKeyChecking=no ./container-installs/docker-install.sh ./webserver-builds/nginx.sh ./virtualization-installs/jenkins-install.sh ./virtualization-installs/aws-terraform.sh $USER@$(multipass info trifecta | grep IPv4 | awk '{ print $2 }'):~/
